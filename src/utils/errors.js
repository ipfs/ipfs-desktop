...
export function fsckError (path, addr) {
  const option = dialog.showMessageBox({
    type: 'warning',
    title: 'IPFS Desktop',
    message: `IPFS Desktop found an api file in ${path}, but was unabled to connect to the address ${addr}. Would you like to try running 'ipfs repo fsck' to remove the file and try again?`,
    buttons: [
      'No, just quit',
